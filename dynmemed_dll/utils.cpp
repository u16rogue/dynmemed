...
bool utils::iat_hook(const char *module_target, const char *import_name, void *hook_ptr, void **orig_ptrptr)
{
	static std::uint8_t *base = reinterpret_cast<std::uint8_t *>(GetModuleHandleA(module_target));
	if (!base)
		return false;
	...
	do
	{
		...
		while (oft->u1.AddressOfData)
		{
			...
			if (oft->u1.Ordinal & IMAGE_ORDINAL_FLAG || strcmp(import_name, n) != 0)
			{
				++oft;
				++ft;
				continue;
